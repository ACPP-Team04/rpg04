...
class EntityFactory {
  ...
  public:
	...
	tson::TiledClass getCustomPropertyAsClass(tson::Property prop) { return prop.getValue<tson::TiledClass>(); }
	...
	void addTilesonComponents(EntityID id, tson::Object obj)
	{
		for (auto &prop : obj.getProperties().getProperties()) {
			...
			spdlog::info("Adding property of type {}", propType);
			...
			spdlog::info("Added property of type {}", propType);
		}
	}
