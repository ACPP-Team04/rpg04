...
void WorldParser::addPersistentIdComponent(ArchetypeManager &manager, EntityID id, tson::Object obj, LEVEL_NAME level)
{
	if (obj.getProperties().hasProperty("uuid")) {
		std::string uniqueId = obj.getProperties().getProperty("uuid")->getValue<std::string>();
		manager.addComponentToEntity<PersistanceComponent>(id);
		manager.getComponent<PersistanceComponent>(id).uuid = uniqueId;
		return;
	}

	if (manager.hasComponent<PlayerComponent>(id)) {
		return;
	}
	...
	} else if (manager.hasComponent<ItemComponent>(id)) {
		needsPersistence = true;
	} else if (manager.hasComponent<InventoryComponent>(id)) {
		needsPersistence = true;
	}

	if (needsPersistence) {
		std::string objType = obj.getType().empty() ? "obj" : obj.getType();
		std::string uniqueId =
		    fmt::format("Lvl{}_{}_{}_{}", static_cast<int>(level), objType, obj.getPosition().x, obj.getPosition().y);
...
void WorldParser::createEntity(std::tuple<tson::Object, LEVEL_NAME> &tuple)
{
	...
	addPersistentIdComponent(manager, id, obj, level);
}
