...
void ItemSystem::update()
{
	...
	this->manager.view<PlayerComponent,InventoryComponent>().each([&](auto entity,PlayerComponent& player,InventoryComponent &inventory) {

		entities.push_back(entity);
	});
	this->manager.view<InteractionComponent,ItemHealingComponent>().each([&](auto entity,InteractionComponent& interaction,ItemHealingComponent &item)
	{
